...
const FichepayScreen = () => {
  const [fichepay, setFichepay] = useState<FichePayItem[]>([]);

  // Charger les fichiers depuis AsyncStorage
  useEffect(() => {
    const loadFiles = async () => {
      try {
        const storedFiles = await AsyncStorage.getItem('fichepay');
        if (storedFiles) {
          setFichepay(JSON.parse(storedFiles));
        }
      } catch (error) {
  ...
  const saveFiles = async (files: FichePayItem[]) => {
    try {
      await AsyncStorage.setItem('fichepay', JSON.stringify(files));
    } catch (error) {
  ...
  const selectPDF = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets[0]) {
        ...
        const newFile: FichePayItem = {
          id: (fichepay.length + 1).toString(),
          name: file.name || 'Fichier sans nom',
          filePath: file.uri,
        };

        const updatedFiles = [...fichepay, newFile];
        setFichepay(updatedFiles); // Met à jour l'état
  ...
  const renderContract = ({ item }: { item: FichePayItem }) => (
